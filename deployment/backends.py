...
def load_backends():
    ...
    if set(backends) != {"eastus2", "sweden", "embedding"}:
        raise ValueError("Expected exactly the configured two chat backends and embedding")
    for name, backend in backends.items():
        ...
        if (
            ...
            or backend["kind"] != ("embedding" if name == "embedding" else "chat")
        ):
...
def backend_url(backend):
    operation = "embeddings" if backend["kind"] == "embedding" else "chat/completions"
    return (
        backend["endpoint"].rstrip("/") + "/openai/deployments/"
        + backend["deployment"] + "/" + operation + "?api-version=" + API_VERSION
    )
