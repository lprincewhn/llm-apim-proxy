...
URL 中的部署名和 body 中的模型名都不转换，必须在当前后端真实存在。East US 2 和 Sweden 聊天部署已统一为 `gpt-5.1`；请求未部署的模型仍会收到上游错误，不会自动转到其他区域。`config.json` 中两地部署名用于控制器直连健康探测及日志归因，不是 APIM 路径映射表。
...
| 错误 | 5 分钟内至少 5 次请求，404、429 或 500–599 错误比例 ≥20% |
...
任一状态码为 401／403 的请求从两条健康告警的样本中排除，避免权限配置错误触发切换；这类错误需另行监控处理。错误率统计网关或后端任一字段的 404／429／500–599。

404 仅在上述已归因的推理路径内计入，包括部署删除后的 `DeploymentNotFound`；任意不存在路径、files/jobs、embedding 不会因此触发聊天切换。规则不读取响应 body，因此同一路径上错误 `api-version` 导致的 404 也可能计入。负向 smoke 应在告警禁用期间运行。达到错误率阈值后仍需备用已启用、两次健康探测通过并满足冷却等保护条件，才切换后续请求；不会自动解除隔离或重试当前失败请求。
