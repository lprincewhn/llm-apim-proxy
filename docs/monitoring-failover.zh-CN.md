...
本文依据 `llm-apim-proxy` 已实现的代码、接口连通记录，以及 2026 年 9 月 15—16 日的真实故障演练编写。运行状态引用截至 9 月 16 日 00:08 UTC 的交付记录，不代表文档阅读时的实时状态。本方案目前为 Azure APIM Developer 实验环境验证结果，不是生产 SLA 承诺。
...
核心价值是将“模型调用”和“故障处置”分开：业务请求直接到达 Foundry，不经过额外的模型执行器；区域选择、健康确认和路由变更集中在网关及控制链路管理，并保留可追溯记录。
...
这里是 HTTP 反向代理，不是逐字节 TCP 隧道；HTTP 传输分帧、逐跳头和 URL 规范化仍遵循 APIM 行为。旧执行器及预算头会被移除。CONNECT、TRACE、WebSocket 和 gRPC 未配置；同一网关上更具体前缀的其他 API 仍可能优先匹配。
...
较早一次删除演练观察约 10 分 05 秒、119 次请求全部成功，未形成可测的故障起点，不能得出切换耗时。9 月 15 日 09:17 的旧演练经过已退役执行器，也不能作为当前直连拓扑的证据。
...
APIM 当前 `forward-request timeout=120` 约束等待响应头，**不是完整响应正文、SSE 空闲时间或业务回合的截止时间**。当前没有意图／改写／生成／embedding 分类预算，也不再解释 `X-Remaining-Budget-Ms`。Logic App 探测同样没有旧执行器的细粒度总读取／空闲预算。
...
项目仓库：<https://github.com/lprincewhn/llm-apim-proxy>。本文实现依据为主线 `460d93a`；文档更新不修改线上策略或资源。

| 依据 | 用途 |
|---|---|
| `deployment/configure_apim.py`、`deployment/llm-policy.xml` | 七种通配操作、鉴权、限流、单次转发及超时边界 |
| `deployment/monitoring/deploy.py` | 日志归属、告警条件、备用探测和 ETag 更新保护 |
| `deployment/README.md` | 接入示例、12:16／12:28 接口记录及部署说明 |
| `deployment/monitoring/drill-20260916.md` | 最新直连拓扑演练摘要、测量口径和证据定位 |
| SVHWB-107，2026-09-16 00:08 UTC 交付附件 `fresh-drill-20260915.json` | 原始请求、管理面活动、告警、工作流动作和收尾状态 |
| `deployment/monitoring/README.md` | 控制器检查、启停和恢复操作手册 |

最新真实切换工作流运行 ID：`08584120898514124676146261226CU43`。原始记录留存于项目问题附件；客户介绍仅列必要摘要，不包含订阅密钥、访问令牌或回调地址。
